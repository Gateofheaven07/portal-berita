      ...
      <nav className="sticky top-0 z-50 bg-white text-slate-900 shadow-md border-b border-slate-200">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16">
            <Link href="/" className="flex-shrink-0 font-bold text-2xl font-publico">
              J.comNews
            </Link>
            <div className="hidden md:flex items-center gap-8">
              {categories.map((cat) => (
                <Link 
                  key={cat.href} 
                  href={cat.href} 
                  className="relative text-sm font-medium text-slate-700 hover:text-slate-900 transition-all duration-300 py-2 px-1 group"
                >
                  {cat.name}
          ...
          <div className="md:hidden pb-4 space-y-2">
            {categories.map((cat) => (
              <Link
                key={cat.href}
                href={cat.href}
                className="block px-4 py-3 hover:bg-slate-100 hover:text-slate-900 rounded transition-all duration-200 font-medium hover:translate-x-1 hover:shadow-sm"
